...
for i, c in enumerate(focuscoils):
    if i < 3:
        ...
        extra_currents.append(Current(c.I))
...
bsextra = BiotSavart(extra_coils)
...
class SquaredFlux(CPPOptimizable):

    def __init__(self, surface, target, field):
        self.surface = surface
        self.target = target
        self.field = field
        CPPOptimizable.__init__(self, x0=np.asarray([]), opts_in=[field])
